...
test('Blog information is present but no operations for anonymous users', () => {
    ...
    expect(screen.queryByRole('button', { name: /Remove blog/i })).not.toBeInTheDocument()

    expect(
        screen.getByText(/Login to interact/i)
    ).toBeInTheDocument()
})
...
test(`The owner are shown both buttons`, () => {
    ...
    expect(screen.getByText((content) => content.includes('25'))).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Like/i })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Remove blog/i })).toBeInTheDocument()
})
